...
export class AdmissionsComponent implements OnInit {
  ...
  getAllAdmissions(): void {
    this.admService.fetchAllAdmissions().subscribe((res: any) => {
      ...
      this.dataSource.data = admissions;
      //   this.setDataSource(admissions);
      this.uploadPath = res.data.uploadPath;
    });
  }

  setDataSource(alldata: []) {
    this.dataSource.data = alldata.map((dt: any) => {
      return dt.personalDetails;
    });
  }
  ...
  displayThumbnail(flyerFileName: string) {
